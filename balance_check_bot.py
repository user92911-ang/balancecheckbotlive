...
async def get_eth_price(session: aiohttp.ClientSession) -> float:
    ...
    except Exception as e: logger.warning(f"Could not fetch ETH price: {e}")
    return 0.0

async def get_native_balance(session: aiohttp.ClientSession, rpc_url: str, address: str, semaphore: asyncio.Semaphore) -> float:
    async with semaphore:
        try:
            ...
            async with session.post(rpc_url, json=payload, timeout=30) as response:
                ...
                else: logger.warning(f"RPC {rpc_url} returned status {response.status} for native balance.")
        except asyncio.TimeoutError: logger.warning(f"RPC {rpc_url} timed out for native balance.")
        except Exception as e: logger.warning(f"RPC {rpc_url} error for native balance: {e}")
    return 0.0

async def get_erc20_balance(session: aiohttp.ClientSession, rpc_url: str, contract: str, address: str, semaphore: asyncio.Semaphore) -> float:
    async with semaphore:
        ...
        try:
            async with session.post(rpc_url, json=balance_payload, timeout=30) as b_resp, session.post(rpc_url, json=decimals_payload, timeout=30) as d_resp:
                ...
                else: logger.warning(f"RPC {rpc_url} returned status {b_resp.status}/{d_resp.status} for ERC20.")
        except asyncio.TimeoutError: logger.warning(f"RPC {rpc_url} timed out for ERC20.")
        except Exception as e: logger.warning(f"RPC {rpc_url} error for ERC20: {e}")
    return 0.0
...
async def get_all_asset_balances(session: aiohttp.ClientSession, addresses: List[str]) -> Dict:
    ...
    for addr in addresses:
        for chain_id, chain_info in CHAINS.items():
            tasks.append(fetch_native(session, chain_info['rpc'], addr, chain_id, chain_info['symbol'], semaphore))
            if chain_id in ERC20_CONTRACTS:
                for symbol, contract in ERC20_CONTRACTS[chain_id].items():
                    tasks.append(fetch_erc20(session, chain_info['rpc'], contract, addr, chain_id, symbol, semaphore))
    ...
    for chain_id, symbol, balance in results:
        if balance and balance > 0.000001:
            if chain_id not in aggregated: aggregated[chain_id] = {}
            if symbol not in aggregated[chain_id]: aggregated[chain_id][symbol] = 0
            aggregated[chain_id][symbol] += balance
            
    return aggregated

async def fetch_native(session, rpc, addr, chain_id, symbol, sem): return chain_id, symbol, await get_native_balance(session, rpc, addr, sem)
async def fetch_erc20(session, rpc, contract, addr, chain_id, symbol, sem): return chain_id, symbol, await get_erc20_balance(session, rpc, contract, addr, sem)
...
async def balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if not addresses:
            await status_message.edit_text("I didn't find any valid addresses or `.eth` names."); return

        await status_message.edit_text(f"✅ Found {len(addresses)} unique address(es). Fetching all balances now, this may take a moment...")
        aggregated_balances = await get_all_asset_balances(session, addresses)
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    if not TELEGRAM_TOKEN: logger.critical("CRITICAL: TELEGRAM_TOKEN not set."); yield; return
    ...
    logger.info("Telegram bot started successfully."); yield
